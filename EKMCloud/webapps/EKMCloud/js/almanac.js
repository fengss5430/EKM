...
(function($) {
    ...
    jQuery.fn.almanac = function(options){
        var Almanac = {
             ...
             * @return null
             */
            drawCld: function (year, month,beforeFn, funct) {
                var i,sD,s,size;
                var cld = this.calendar(year, month);
                //外部修改cld
                typeof beforeFn=="function"&&beforeFn.call(this,cld);
                this.cld = cld;
                ...
                for (i = 0; i < 42; i++) {
                    ...
                    if (sD > -1 && sD < cld.length) {  //日期内
                         /*判断状态开始*/
                         if(cld.statusArr[sD].status<2&&cld.statusArr[sD].userStatic>0){
                             eDay.addClass('mh-rest');
                             eDay.addClass('mh-book');
                         }else if(cld.statusArr[sD].status<2){
                         	eDay.addClass('mh-rest');
                         }else if(cld.statusArr[sD].status>=2&&cld.statusArr[sD].userStatic>0){
                         	eDay.addClass('mh-busy');
                         	 eDay.addClass('mh-book');
                         }else if(cld.statusArr[sD].status>=2){
                         	eDay.addClass('mh-busy');
                         }else{
                        ...
                        if (cld[sD].isToday) {      // 今日
                            ...
                            //如果当天被预约，则将状态标记
                         cld.statusArr[sD].isBook===true&&eDay.addClass("mh-book");
                        }
            ...
            showAlmanacDetail: function (sObj) {
                ...
                if (sObj.attr('data-solor')) {
                    ...
                    var dataTime=cld[d].sYear + '-' + cld[d].sMonth + '-' + cld[d].sDay;
...
//                			"platform":platform
                			 ...
                			 if(result.status == 0){
                				 ...
                				 for(var i = 0; i < list.length; i++){
                						if(list[i].startTime!=null){
                							var startTime=list[i].startTime.substring(0, list[i].startTime.lastIndexOf("."));
                						}
                						 html+='<a href="javascript:;" id="'+list[i].projectId+'" name="resource-details" onclick="ChangePage(this);"><p>会议室名称<span>:'+list[i].platformname+'</span></p><p>项目名称<span>:'+list[i].projectName+'</span></p><p>姓名<span>:</span><span>'+list[i].audituserName+'</span></p><p><span>开始时间</span><span>:</span><span>'+startTime+'</span></p></a>';
                     			}
                				 $("#todaylist").html(html);
                			 }
                		}
            ...
            initControlBar: function(){
                ...
                // console.log( $(".month-list").html(htmlMonth));
                yearmonth=this.today.tY()+"-"+tM;
                // 绑定事件
        ...
        var DateSelection = {
            ...
            changeView : function() {
                ...
                Almanac.clear();
                Almanac.drawCld(global.currYear, global.currMonth, global.beforeDrawCld, global.afterDrawCld);
            },
            /**
             * 刷新工具栏
             */
        ...
        return this.each(function(){
            ...
            Almanac.drawCld(
                Almanac.today.tY(),
                Almanac.today.tM(),
                global.beforeDrawCld,
                global.afterDrawCld);
            if (global.callFunc) global.callFunc();
            /*年份递减*/
            $("#nianjian").unbind('click').bind('click', function() {
                var month=$(".month-list .on").attr('val');
                var year=$(".year-list .on").attr('val');
                year=parseInt(year);
            ...
            $(".year-list li").unbind('click').bind('click', function() {
                var _this = $(this);
                var year=_this.attr('val');
                var month=$("#mothid").attr('val');
                yearmonth=year+"-"+month;
            ...
            $(".month-list li").unbind('click').bind('click', function() {
                var _this = $(this);
                var month=_this.attr('val');
                var year=$(".on").attr('val');
                yearmonth=year+"-"+month;
                _this.parent().addClass('hidden');
